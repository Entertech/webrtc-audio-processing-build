...
if [[ -z "$1" ]]; then
  echo "Usage: $0 'debug' | 'release' 'source_dir' 'out_dir'"
  exit 0
fi
...
echo "agc2/xcframework.sh: MODE=$MODE, DEBUG=$DEBUG, SOURCE_DIR=$SOURCE_DIR, OUT_DIR=$OUT_DIR"
...
for platform_config in "${PLATFORMS[@]}"; do
  ...
  gn gen "$OUT_DIR/$platform" --args="$COMMON_ARGS $config"
...
lipo -create -output "$OUT_DIR/macOS-lib/$FRAMEWORK_NAME.framework/Versions/A/$FRAMEWORK_NAME" \
  "$OUT_DIR/macOS-arm64/$FRAMEWORK_NAME.framework/Versions/A/$FRAMEWORK_NAME" \
  "$OUT_DIR/macOS-x64/$FRAMEWORK_NAME.framework/Versions/A/$FRAMEWORK_NAME"
if [ -d "$OUT_DIR/macOS-x64/$FRAMEWORK_NAME.dSYM" ]; then
  ...
  lipo -create -output "$OUT_DIR/macOS-lib/$FRAMEWORK_NAME.dSYM/Contents/Resources/DWARF/$FRAMEWORK_NAME" \
    "$OUT_DIR/macOS-arm64/$FRAMEWORK_NAME.dSYM/Contents/Resources/DWARF/$FRAMEWORK_NAME" \
    "$OUT_DIR/macOS-x64/$FRAMEWORK_NAME.dSYM/Contents/Resources/DWARF/$FRAMEWORK_NAME"
fi
...
if [ -d "$OUT_DIR/iOS-arm64-simulator/$FRAMEWORK_NAME.dSYM" ]; then
  ...
  lipo -create -output "$OUT_DIR/iOS-simulator-lib/$FRAMEWORK_NAME.dSYM/Contents/Resources/DWARF/$FRAMEWORK_NAME" \
    "$OUT_DIR/iOS-arm64-simulator/$FRAMEWORK_NAME.dSYM/Contents/Resources/DWARF/$FRAMEWORK_NAME" \
    "$OUT_DIR/iOS-x64-simulator/$FRAMEWORK_NAME.dSYM/Contents/Resources/DWARF/$FRAMEWORK_NAME"
fi
...
if [[ -f "$FRAMEWORK_NAME" ]] && [[ ! -L "$FRAMEWORK_NAME" ]]; then
